...
submissionsActions.getProcessingSubmissions.listen(
  (assetUid: string, questionsPaths: string[]) => {
    let fields = '';
...
submissionsActions.getSubmission.listen(
  (assetUid: string, submissionId: string) => {
    dataInterface
...
submissionsActions.getSubmissionByUuid.listen(
  (assetUid: string, submissionUuid: string) => {
      ...
      .done((response: PaginatedResponse<SubmissionResponse>) => {
        ...
        submissionsActions.getSubmissionByUuid.completed(
          response.results.find(
            (sub) => sub['meta/rootUuid'] === submissionUuid
          ) || response.results[0]
        );
      })
...
submissionsActions.bulkDeleteStatus.listen(
  (uid: string, data: BulkSubmissionsRequest) => {
    dataInterface
...
submissionsActions.bulkPatchStatus.listen(
  (uid: string, data: BulkSubmissionsRequest) => {
    dataInterface
 ...
 */
submissionsActions.bulkPatchValues.listen(
  (uid: string, submissionIds: string[], data: BulkSubmissionsRequest) => {
    dataInterface
...
submissionsActions.bulkPatchValues.completed.listen(
  (response: {failures: number}) => {
