...
bem.LanguageSelector__sourceLanguage = makeBem(
  bem.LanguageSelector,
  'source-language'
);
bem.LanguageSelector__sourceLabel = makeBem(
  bem.LanguageSelector,
  'source-label',
  'label'
);
bem.LanguageSelector__searchBoxRow = makeBem(
  bem.LanguageSelector,
  'search-box-row'
);
bem.LanguageSelector__searchBoxWrapper = makeBem(
  bem.LanguageSelector,
  'search-box-wrapper'
);
bem.LanguageSelector__searchBox = makeBem(bem.LanguageSelector, 'search-box');
bem.LanguageSelector__searchBoxLabel = makeBem(
  bem.LanguageSelector,
  'search-box-label',
  'label'
);
bem.LanguageSelector__searchBoxInput = makeBem(
  bem.LanguageSelector,
  'search-box-input',
  'input'
);
bem.LanguageSelector__clearSearchBox = makeBem(
  bem.LanguageSelector,
  'clear-search-box',
  'button'
);
bem.LanguageSelector__selectedLanguage = makeBem(
  bem.LanguageSelector,
  'selected-language'
);
bem.LanguageSelector__selectedLanguageLabel = makeBem(
  bem.LanguageSelector,
  'selected-language-label'
);
bem.LanguageSelector__clearSelectedLanguage = makeBem(
  bem.LanguageSelector,
  'clear-selected-language',
  'button'
);
bem.LanguageSelector__notFoundMessage = makeBem(
  bem.LanguageSelector,
  'not-found-message',
  'li'
);
bem.LanguageSelector__helpBar = makeBem(
  bem.LanguageSelector,
  'help-bar',
  'footer'
);

const LANGUAGE_SELECTOR_SUPPORT_URL =
  'transcription-translation.html#language-list';
...
> {
  ...
  openSupportPage() {
    window.open(
      envStore.data.support_url + LANGUAGE_SELECTOR_SUPPORT_URL,
      '_blank'
    );
  }
  ...
  async fetchSourceLanguage() {
    ...
    if (this.props.sourceLanguage) {
      try {
        const language = await languagesStore.getLanguage(
      ...
      this.props.suggestedLanguages.forEach(async (languageCode) => {
        try {
          ...
          const isAlreadyAdded = Boolean(
            this.state.suggestedLanguages?.find(
              (stateLanguage) => stateLanguage.code === language.code
            )
          );
          if (
    ...
    return (
      <li key={language.code}>
        <Button
          ...
          size='m'
          label={
            <LanguageDisplayLabel code={language.code} name={language.name} />
          }
